#!/bin/bash
# ----------------------------------------------------
# SCRIPT IDS FINAL (Dijalankan di CORE ROUTER)
# ----------------------------------------------------

# HARI INI: Tanggal wajib untuk log
date -s "2025-12-08 12:00:00"

# INTERFACE: Tentukan SEMUA interface yang harus dimonitor
# eth1=Akademik, eth2=Riset, eth3=Mahasiswa, eth4=Admin, eth5=Guest
INT_LIST="eth1 eth2 eth3 eth4 eth5"
...
outputs:
  ...
  - eve-log:
      ...
      types: [alert]
af-packet:
# Menggunakan satu entry dan list interface untuk memonitor semua segment
  - interface: \$INT_LIST
    cluster-id: 99
...
# 2. SSH Brute Force ke Server Riset (10.20.30.10)
...
for INT in \$INT_LIST; do
    ethtool -K \$INT rx off tx off sg off gso off gro off 2>/dev/null || true
done
echo "✅ IDS SIAP (Rules Optimized)!"

# Jalankan Suricata
suricata -c /etc/suricata/suricata.yaml -i \$INT_LIST -D